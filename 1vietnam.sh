...
find /usr/local/assp/log -mtime -31 -name *bmail*gz  | while read line
do
...
cat /tmp/.miki.txt  | sort -k 1,1  |  sort -n | awk '{print $3" "$2" "$1}'  | uniq -c -f 1 | awk -v limit=30 '$1 > limit{print $3}' |  grep -wFf- /tmp/.miki.txt > /tmp/miki.txt
...
cat /opt/megaspam/vietnamip/domains.txt | grep -Ff- /tmp/miki2.txt | cut -d ' ' -f1 | sort | uniq > /opt/megaspam/vietnamip/whitelist.out
